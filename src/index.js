...
io.on('connection', (socket) => {
    ...
    socket.on('join', ({ username, room }, callback) => {
        ...
        socket.broadcast.to(user.room).emit("message", messages.generateMessage('Admin', `${user.username} has joined!`))
    ...
    socket.on("sendLocation", (position, callback) => {
        ...
        io.to(user.room).emit("locationMessage", messages.generateLocationMessage(user.username, `https://google.com/maps?q=${position.latitude},${position.longitude}`))
        callback()
    })
